...
def faceCluster():
    ...
    def getFiles():
        ...
        path = '/Users/artrial/desktop/face_detect/face_clustering/FaceImageClustering/DataSet'

        for filename in os.listdir(os.getcwd()+"/DataSet/"):
            print(str(filename))
            filename1 = os.getcwd()+"/DataSet/"+str(filename)
            if '.jpg' in filename1:
                imagesTitles.append(filename1)
    ...
    def getFaces(images):
        ...
        profileCascade = cv2.CascadeClassifier('classifier/haarcascade_profileface.xml')
        faces = list()
        ...
        for image in images:
            ...
            profileFaces = profileCascade.detectMultiScale(image, scaleFactor=1.3, minNeighbors=5, minSize=(30, 30), flags = cv2.CASCADE_SCALE_IMAGE)
    ...
    def graphSubClusters(faces, similarityMatrix, labels):
        for k,v in groupLabels(labels).iteritems():
            # print('cluster {0}'.format(k) + " 155 cluster")
    ...
    def saveToFile(faces, labels, pretense='original'):
        for k,v in groupLabels(labels).iteritems():
            # print('cluster {0}'.format(k) + "clusters")
            if not os.path.exists("results/cluster{0}".format(k)):
                os.makedirs("results/cluster{0}".format(k))
            for i in v:
                # print(str(faces[i]))
                Image.fromarray(faces[i]).save('/Users/artrial/desktop/face_detect/face_clustering/FaceImageClustering/results/cluster{0}/cluster{1}_{2}_{3}.jpg'.format(k,k,i, pretense))
